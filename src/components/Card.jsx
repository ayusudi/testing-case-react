export default function Component({ el }) {
  return (
    <div className="card w-25 m-4 d-flex flex-col gap-2" >
      ...
      <h5 className="px-3 card-title">{el.name}</h5>
      <div className='px-3 d-flex align-items-center gap-2'>
        ...
        <p className='m-0'>{el.rating}</p>
      </div>
      <ul className="list-group list-group-flush">
        {
          el.tags.slice(0, 2).map(tag => <li className="list-group-item">{tag}</li>)

        }
      </ul>
      <a href={`https://dummyjson.com/recipes/${el.id}`} className='px-3 pb-2 text-warning'>READ MORE</a>
    </div>
  )
}